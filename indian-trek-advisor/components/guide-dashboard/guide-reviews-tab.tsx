...
export function GuideReviewsTab({
  reviews,
  guideRating,
  guideTotalRatings,
}: {
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card className="border-border/60 bg-card/60 backdrop-blur-xl">
          <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:gap-6 sm:p-6">
            <div className="text-center">
              ...
              <StarRating rating={Math.round(guideRating)} />
              <p className="mt-1 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                {guideTotalRatings} {guideTotalRatings === 1 ? "rating" : "ratings"}
              </p>
            </div>
        ...
        <Card className="border-border/60 bg-card/60 backdrop-blur-xl">
          <CardContent className="flex flex-col items-center justify-center p-6">
            ...
            <p className="mt-3 font-mono text-2xl font-bold">{realCount}</p>
        ...
        <CardContent>
          ...
          >
            {reviews.length === 0 ? (
              ...
              reviews.map((review) => {
                const trek = review.booking?.trek_id ? getTrekById(Number(review.booking.trek_id)) : undefined
                return (
                  <motion.div
                    key={review.id}
                    variants={item}
